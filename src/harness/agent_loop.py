...
Gemini Model Fallback Cascade (gemini-3.6 -> 3.5 -> 3.1 -> gemma-4) is preserved.
"""
import json
import logging
import asyncio
...
def get_current_time_context() -> str:
    """Tự động lấy thời gian hệ thống thực tế và format chuẩn tiếng Việt"""
    from datetime import datetime
    now = datetime.now()
    weekdays = ["Thứ Hai", "Thứ Ba", "Thứ Tư", "Thứ Năm", "Thứ Sáu", "Thứ Bảy", "Chủ Nhật"]
    weekday_name = weekdays[now.weekday()]
    return f"{weekday_name}, {now.strftime('%d/%m/%Y %H:%M:%S')}"
...
class HarnessAgentLoop:
    ...
    def _compact_context_if_needed(history: str, max_turns: int = 8) -> str:
        ...
        summary_line = f"[TÓM TẮT LỊCH SỬ CŨ ({len(old_turns)//2} lượt thoại): Người dùng đã hỏi và nhận thông tin báo cáo tài chính/công nợ trước đó.]"
        return summary_line + "\n" + "\n".join(recent_turns)
...
Hãy suy luận và sử dụng các Tool cần thiết để thu thập số liệu chính xác nhất trước khi trả lời."""
        ...
        raise RuntimeError(f"Harness Agent Loop vượt quá số bước tối đa ({MAX_AGENT_STEPS}) mà chưa hoàn thành.")
